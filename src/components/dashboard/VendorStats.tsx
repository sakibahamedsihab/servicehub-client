...
export function VendorStats() {
  const [stats, setStats] = useState({ earnings: 0, upcoming: 0 });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function fetchStats() {
      try {
        const res = await bookingsApi.getVendorBookings({ limit: 1000 });
        ...
        for (const booking of bookings) {
          if (booking.status === "completed") {
            try {
              const service = await servicesApi.getById(booking.serviceId);
        ...
        setStats({ earnings: totalEarnings, upcoming: upcomingCount });
      } catch (err) {
